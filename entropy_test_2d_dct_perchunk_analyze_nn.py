...
if __name__ == '__main__':
  original_data = torch.load('/home/yujin-wa20/projects/LoftQ/output/mistral/base_model.model.model.layers.0.mlp.up_proj.lora_A.default.pt')[0]
  ...
  D = get_dct_matrix(8)
  correlation_coefficient_list = np.zeros((original_data_col // 8, original_data_row // 8))
  ...
  for i in range(0, original_data_col, 8):
    for j in range(0, original_data_row, 8):
      ...
      idx = np.arange(63) / 63
      zigzag_matrix = zigzag_matrix[1:] # delete DC component

      correlation_coefficient = np.corrcoef(idx, zigzag_matrix)[0, 1]
      correlation_coefficient_list[i // 8][j // 8] = correlation_coefficient
